...
class NoisyDataset(Dataset):
    def __init__(self, root_folder, transform=None):
        ...
        self.images = os.listdir(root_folder)
        self.transform = transform
    ...
    def __getitem__(self, idx):
        ...
        image_src = cv2.cvtColor(read_exr(self.root_folder + '/' + fname), cv2.COLOR_BGR2RGB)
        ...
        image_mid = images[1]
        image_high = images[2]
        image_ulaw = np.log10(1.0 + u * image_hdr) / np.log10(1.0 + u)
        images_original = image_hdr

        if self.transform:
            image_low = self.transform(image_low)
            image_mid = self.transform(image_mid)
            image_high = self.transform(image_high)
            image_ulaw = self.transform(image_ulaw)
            images_original = self.transform(images_original)

        return image_low, image_mid, image_high, image_ulaw, images_original


def normalize_gaussian(feature_image, kernel_size, sigma, device):
    ...
    kernel_tensor = torch.from_numpy(kernel).unsqueeze(0).unsqueeze(0).repeat(feature_image.shape[1], 1, 1, 1).detach().to(device)
...
def calculate_local_mean(input_img, kernel_size, device):
    mean_filter = torch.ones(input_img.shape[1], 1, kernel_size, kernel_size).to(device)
...
def calculate_feature_contrast_masking(gauss_out, mean_out, u_law_processing):
...
def restore_color_from_luminance(gt_img, pred_lum, a=0.6):
    r, g, b = gt_img[0], gt_img[1], gt_img[2]
    y_gt_lum = compute_luminance(gt_img) + EPSILON
...
class TMAP(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for feature_prediction, feature_ulaw in zip(self.vgg(prediction), self.vgg(img_ulaw)):
            prediction_gauss = normalize_gaussian(feature_prediction, kernel_size=13, sigma=2, device=self.device)
            prediction_means = calculate_local_mean(feature_prediction, kernel_size=13, device=self.device)
            feature_prediction_mask = calculate_feature_contrast_masking(prediction_gauss, prediction_means, u_law_processing=False)
            ulaw_gauss = normalize_gaussian(feature_ulaw, kernel_size=13, sigma=2, device=self.device)
            ulaw_means = calculate_local_mean(feature_ulaw, kernel_size=13, device=self.device)
            feature_ulaw_mask = calculate_feature_contrast_masking(ulaw_gauss, ulaw_means, u_law_processing=True)
        ...
        with torch.no_grad():
            ...
            pred_lum = compute_luminance(pred)
            restored_img = restore_color_from_luminance(gt, pred_lum)
            if self.i % 10 == 0:
                img = restored_img / torch.max(restored_img)
